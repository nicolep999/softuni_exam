...
class Command(BaseCommand):
    ...
    def generate_contextual_review(self, movie):
        ...
        review_data = self.get_genre_specific_review(rating_category, genres, movie.title, imdb_rating)
    ...
    def get_genre_specific_review(self, rating_category, genres, movie_title, imdb_rating):
        ...
        if rating_category == 'excellent':
            titles = [
                "A true masterpiece!", "Absolutely brilliant!", "Cinematic perfection!",
                "Outstanding achievement", "A work of art", "Exceptional filmmaking",
                "Pure cinematic magic", "A masterpiece of storytelling", "Incredible film",
        ...
        # Very good movies (7.5-8.4)
            ...
            titles = [
                "Great film!", "Highly recommended!", "Excellent movie!",
                "Really enjoyed this", "Well-crafted film", "Strong performances",
                "Engaging story", "Quality filmmaking", "Worth watching",
                "Impressive work"
            ]
        ...
        # Good movies (6.5-7.4)
            ...
            titles = [
                "Good movie", "Enjoyable watch", "Decent film",
                "Worth a look", "Not bad", "Has its moments",
                "Pretty good", "Decent entertainment", "Okay film",
                "Fairly enjoyable"
            ]
        ...
        # Average movies (5.5-6.4)
            ...
            titles = [
                "Mediocre", "Average at best", "Nothing special",
                "Forgettable", "Meh", "Could be better",
                "Not great", "Disappointing", "Lacks substance",
                "Underwhelming"
            ]
        ...
        # Poor movies (below 5.5)
            ...
            titles = [
                "Not good", "Poor quality", "Waste of time",
                "Terrible", "Bad film", "Avoid this",
                "Disappointing", "Awful", "Boring",
                "Hard to watch"
            ]
        ...
        if 'action' in genres:
            if rating_category in ['excellent', 'very_good']:
                contents.append(f"'{movie_title}' delivers fantastic action sequences with great choreography and impressive stunts. The action scenes are well-shot and genuinely exciting.")
            elif rating_category in ['good', 'average']:
                contents.append(f"The action in '{movie_title}' is decent enough, though nothing particularly groundbreaking. The fight scenes are okay but could have been better executed.")
            else:
                contents.append(f"The action in '{movie_title}' is poorly choreographed and unconvincing. The fight scenes are boring and lack any real excitement.")
        
        if 'comedy' in genres:
            if rating_category in ['excellent', 'very_good']:
                contents.append(f"'{movie_title}' is genuinely funny with clever humor and great comedic timing. The jokes land well and the comedy feels natural and entertaining.")
            elif rating_category in ['good', 'average']:
                contents.append(f"The comedy in '{movie_title}' has some funny moments, though not all jokes land. There are some decent laughs but it's inconsistent.")
            else:
                contents.append(f"The humor in '{movie_title}' falls flat. The jokes are unfunny and the comedy feels forced and awkward.")
        
        if 'drama' in genres:
            if rating_category in ['excellent', 'very_good']:
                contents.append(f"'{movie_title}' delivers powerful dramatic moments with emotional depth and strong character development. The dramatic elements are well-handled and moving.")
            elif rating_category in ['good', 'average']:
                contents.append(f"The dramatic elements in '{movie_title}' are handled reasonably well, though they could have been more impactful. The emotional moments are okay but not particularly moving.")
            else:
                contents.append(f"The dramatic elements in '{movie_title}' feel forced and unconvincing. The emotional moments lack authenticity and fail to engage.")
        
        if 'horror' in genres:
            if rating_category in ['excellent', 'very_good']:
                contents.append(f"'{movie_title}' creates genuine tension and scares with effective atmosphere and well-crafted horror elements. The film is genuinely frightening and well-executed.")
            elif rating_category in ['good', 'average']:
                contents.append(f"The horror elements in '{movie_title}' are decent, with some effective scares and atmosphere. It's not particularly frightening but has its moments.")
            else:
                contents.append(f"The horror in '{movie_title}' is ineffective and not scary at all. The scares are predictable and the atmosphere is lacking.")
