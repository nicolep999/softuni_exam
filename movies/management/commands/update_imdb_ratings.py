...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not omdb_api_key:
            self.stdout.write(self.style.ERROR('OMDB API key is required. Set OMDB_API_KEY environment variable or use --omdb-key'))
            self.stdout.write(self.style.WARNING('You can get a free OMDB API key from: http://www.omdbapi.com/apikey.aspx'))
            return
        ...
        for movie in movies:
            try:
                ...
                movie_data = self.search_movie_on_tmdb(tmdb_api_key, movie.title, movie.release_year)
        ...
        self.stdout.write(
            self.style.SUCCESS(f'Update completed! Updated {updated_count} movies with proper IMDb ratings')
        )
