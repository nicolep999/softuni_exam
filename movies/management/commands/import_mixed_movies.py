...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"Starting import of {top_rated_pages} pages from top-rated movies and {new_movies_pages} pages from newer movies..."
        )
        ...
        for page in range(1, top_rated_pages + 1):
            ...
            for movie_data in movies_data:
                try:
                    ...
                    self.stdout.write(
                        self.style.ERROR(
                            f'❌ Error importing {movie_data.get("title", "Unknown")}: {e}'
                        )
                    )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Import completed! Successfully imported {total_movies_imported} movies"
            )
        )
    ...
    def import_movie(self, api_key, movie_data):
        ...
        if Movie.objects.filter(
            title=movie_data["title"], release_year=movie_data["release_date"][:4]
        ).exists():
            return None
            ...
            if detailed_data.get("release_date") and not release_date:
                try:
                    movie.release_date = datetime.strptime(
                        detailed_data["release_date"], "%Y-%m-%d"
                    ).date()
                except ValueError:
                    pass
